...
namespace test\Drupal\page_cache\StackMiddleware;
 ...
 */
class test34343PageCache implements HttpKernelInterface {
...
//------ bona -----
    // DELETE THE ALLOW VARIABLE FROM THE ORIGINAL IF STATEMENT
    // In the cases below we deny caching of the page.
    ...
    if ($count > 2) {
      $alias = '/fresh';
      $query = "SELECT source FROM url_alias WHERE alias = :alias";
      $source = \Drupal\Core\Database\Database::getConnection()
        ->query($query, array(':alias' => $alias))
        ->fetchField();
      ...
      if (strpos($source, '/node/') === 0) {
        ...
        if (is_numeric($nid)) {
          $industry_field = \Drupal\ips\IndustryManagerInterface::TERM_INDUSTRY_FIELD;
          $table = "node__{$industry_field}";
          $query = "SELECT ind.field__term_industry_target_id as industry from node_field_data n LEFT JOIN $table ind
ON n.nid = ind.entity_id WHERE n.type = 'industry' AND ind.entity_id = :nid";
          $industry = \Drupal\Core\Database\Database::getConnection()
            ->query($query, array(':nid' => $nid))
            ->fetchField();

          if(!empty($industry)) {
            // Set the industry cookie when industry node changes.
            // It is done here before the page is cached.
            // If note done here the cookie will never be updated.
            $params = session_get_cookie_params();
//            setcookie('industry', $industry, REQUEST_TIME + \Drupal\site_entry\SiteEntryManagerInterface::COOKIE_LIFETIME, $params['path'], $params['domain'], $params['secure'], $params['httponly']);
          }
    ...
    $temp_store = \Drupal::service('user.shared_tempstore')->get('global');
    $gg = 0;
// ------------
//    $url = \Drupal\Core\Url::fromUserInput($request->getPathInfo());
    //-----------------------
    // Only allow page caching on master request.
    if ($allow && $type === static::MASTER_REQUEST && $this->requestPolicy->check($request) === RequestPolicyInterface::ALLOW) {
